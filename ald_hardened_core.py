"""Hardened public boundary for the deterministic ALD core.

This module is the phase-one hardening layer. The top-level
``ald_media_controller`` module composes this boundary with media codecs.
"""
...
def _harden_validated_recipe(recipe):
    instructions = tuple(
        MappingProxyType({"opcode": instruction["opcode"], "arguments": _deep_freeze_validated(instruction["arguments"])})
...
def _normalized_json_is_exact(value: Any) -> bool:
    ...
    if type(value) is MappingProxyType:
        try:
            return all(type(key) is str and _normalized_json_is_exact(item) for key, item in value.items())
        except Exception:
            return False
    return False
...
def _limits_are_exact(limits: Any) -> bool:
    ...
    try:
        return (
            type(limits.min_purge_ms) is int and limits.min_purge_ms >= 1
            and type(limits.max_temperature_c) is float and math.isfinite(limits.max_temperature_c) and limits.max_temperature_c > 0
            and type(limits.max_pressure_pa) is float and math.isfinite(limits.max_pressure_pa) and limits.max_pressure_pa > 0
            and type(limits.max_cycles) is int and limits.max_cycles >= 1
            and type(limits.max_runtime_ms) is int and limits.max_runtime_ms >= 1
            and type(limits.max_residual_fraction) is float and math.isfinite(limits.max_residual_fraction) and 0.0 <= limits.max_residual_fraction <= 1.0
            and type(limits.max_packet_bytes) is int and 1 <= limits.max_packet_bytes <= 800
        )
    except Exception:
        return False
...
def _recipe_shape_is_trusted(recipe: Any) -> bool:
    ...
    try:
        ...
        for name in ("A", "B"):
            ...
            if type(precursor) is not MappingProxyType or set(precursor.keys()) != {"label"} or type(precursor["label"]) is not str or not precursor["label"]:
                return False
        if type(recipe.initial_conditions) is not MappingProxyType or set(recipe.initial_conditions.keys()) != {"temperature_c", "pressure_pa"}:
            return False
        temperature = recipe.initial_conditions["temperature_c"]
        pressure = recipe.initial_conditions["pressure_pa"]
        if type(temperature) is not float or not math.isfinite(temperature) or type(pressure) is not float or not math.isfinite(pressure):
            return False
        ...
        for instruction in recipe.instructions:
            ...
            if type(opcode) is not str or type(arguments) is not MappingProxyType or not _normalized_json_is_exact(arguments):
                return False
...
def _hardened_start_run(self, compiled, seed):
    ...
    self.chamber = _base.VirtualChamber(simulation_time_ms=0, temperature_c=25.0, pressure_pa=101325.0)
    self._audit = []
    self._cycles = []
    ...
    self.chamber = _base.VirtualChamber(simulation_time_ms=0, temperature_c=recipe.initial_conditions["temperature_c"], pressure_pa=recipe.initial_conditions["pressure_pa"])
...
def _strict_surface_post_init(self) -> None:
    ...
    for field in ("blocked_fraction", "defect_fraction", "k_a", "k_b", "growth_nm_per_reaction_fraction"):
...
def _hardened_handle_fault(self, error) -> None:
    ...
    self._fault = _base.FaultRecord(code=error.code, packet_sequence=packet_sequence, state=fault_state, last_verified_digest=self._last_verified_digest, chamber=self._chamber_snapshot(), interlocks=self.interlocks)
    try:
        if self.state is not _base.ControllerState.FAULT:
            self.transition(_base.ControllerState.FAULT, packet_sequence=packet_sequence, event_type="FAULT", details={"code": error.code})
    except BaseException:
        self.state = _base.ControllerState.FAULT
    try:
        self.transition(_base.ControllerState.SHUTDOWN, packet_sequence=packet_sequence, event_type="SHUTDOWN")
    except BaseException:
    ...
    try:
        ...
        self._record("SHUTDOWN_COMPLETE" if shutdown_error is None else "SHUTDOWN_FAULT_CONTAINED", packet_sequence, details)
    except BaseException:
        pass
...
def _emit_cli_error(error: BaseException, exit_code) -> int:
    payload = {"error": {"type": type(error).__name__, "code": exit_code.name, "exit_code": int(exit_code), "message": str(error)}}
...
def main(argv=None) -> int:
    ...
    try:
        ...
        if arguments.command == "simulate":
            return _base._run_simulate(arguments.recipe, arguments.seed, arguments.output, arguments.overwrite)
        ...
        if log_level == "DEBUG":
            traceback.print_exc()
        exit_code = _base.ExitCode.RECIPE if arguments.command == "validate" else _base.ExitCode.CONTROLLER if arguments.command == "simulate" else _base.ExitCode.USAGE
