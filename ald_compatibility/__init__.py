...
The core evidence implementation remains in the repository root module so its
...
# intentionally defined after this export so they can extend the core cleanly.
...
def _recipe_by_id(snapshot: Any) -> dict[str, dict[str, Any]]:
    return {
...
def _effective_pair_score(pair: dict[str, Any] | Any) -> float:
    """Shrink sparse pair evidence toward neutral before candidate aggregation."""
    raw = max(0.0, min(100.0, float(pair.get("score", 50.0))))
    coverage = max(0.0, min(1.0, float(pair.get("coverage", 0.0))))
    return 50.0 + (raw - 50.0) * coverage
...
def _entity_matches_search(entity: dict[str, Any], search: str) -> bool:
    ...
    return any(
        needle in value or formula_needle in value.casefold()
...
def _candidate_evidence_level(
    ...
    *,
    ...
    recipes = _recipe_by_id(snapshot)
    for recipe_id in matching_recipe_ids:
        recipe = recipes.get(recipe_id, {})
        if (
            str(recipe.get("chemistry_status", "")) in _DIRECT_STATUSES
            and bool(recipe.get("source_references"))
        ):
            return "E4_DIRECT"
    ...
    if (matching_recipe_ids or subset_recipe_ids) and any(
        _EVIDENCE_ORDER.get(level, 0) >= _EVIDENCE_ORDER["E1_HEURISTIC"]
...
def _evaluate_candidate(
    ...
    entities_by_id = {
        str(item["id"]): item for item in snapshot.get("precursors", [])
    }
    ...
    effective_scores = [_effective_pair_score(pair) for pair in pairs]
    harmonic = _harmonic_mean(effective_scores)
    minimum = min(effective_scores) if effective_scores else 0.0
    coverage = (
    ...
    matching_recipe_ids = [
        recipe_id
        for recipe_id, recipe_set in recipe_sets
        if recipe_set == candidate_set
    ]
    subset_recipe_ids = [
        recipe_id
        for recipe_id, recipe_set in recipe_sets
        if candidate_set < recipe_set
    ]
    known_support = (
        100.0
        if matching_recipe_ids
        else (60.0 if subset_recipe_ids else 0.0)
    )
    components = {
    ...
    evidence_level = _candidate_evidence_level(
        snapshot,
        pairs,
        matching_recipe_ids,
        subset_recipe_ids,
    ...
    return {
        ...
        "subset_recipe_ids": subset_recipe_ids,
        "components": components,
        "weakest_pair": weakest_pair,
        "weakest_pair_effective_score": (
            round(_effective_pair_score(weakest_pair), 6)
...
def explain_candidate(
    snapshot: Any, precursors: list[str] | tuple[str, ...]
) -> dict[str, Any]:
...
def _beam_sort_key(candidate: dict[str, Any]) -> tuple[Any, ...]:
    return (
...
def _final_sort_key(candidate: dict[str, Any]) -> tuple[Any, ...]:
    return (
        -float(candidate["score"]),
...
def rank_candidates(
    ...
    *,
    ...
    if (
        minimum_evidence not in _EVIDENCE_ORDER
        or minimum_evidence == "E_CONFLICT"
    ):
    ...
    entities = sorted(
        snapshot.get("precursors", []), key=lambda item: str(item["id"])
    )
    entity_ids = [str(item["id"]) for item in entities]
    position = {entity_id: index for index, entity_id in enumerate(entity_ids)}
    ...
    for left_index in range(len(entity_ids)):
        for right_index in range(left_index + 1, len(entity_ids)):
            ...
            candidate = _evaluate_candidate(
                snapshot, ids, pair_index, recipe_sets, weights
            )
    ...
    for size in range(2, max_size + 1):
        if size > 2:
            ...
            for parent in beam:
                parent_ids = tuple(
                    sorted(str(item["id"]) for item in parent["precursors"])
                )
                last_position = position[parent_ids[-1]]
            ...
            beam = sorted(
                expanded.values(), key=_beam_sort_key
            )[:beam_width]
        ...
        for candidate in beam:
            ...
            if float(candidate["score"]) < float(minimum_score):
                continue
            if (
                _EVIDENCE_ORDER[candidate["evidence_level"]]
                < _EVIDENCE_ORDER[minimum_evidence]
            ):
                continue
            if search_text and not (
                {str(item["id"]) for item in candidate["precursors"]}
                & matching_search_ids
            ):
                continue
    ...
    ranked = sorted(unique.values(), key=_final_sort_key)
    return ranked[:top]
